...
function throttle<A extends unknown[]>(fn: (...args: A) => void, wait: number) {
  ...
  // TODO: two nodes in different trees have no common ancestor — detachment alone does not
...
function mergeBlocks(blocks: Block[]): Block[] {
  ...
  return [];
    ...
    answer: [
      ...
      'The edge case that breaks a naive version is the empty list: seeding current with the first element throws or returns a block of undefined, so handle it before the loop. An inverted block, end before start, is the other bad-data one — it is not an interval, and feeding it in makes the sweep extend the current end backwards or swallow the blocks after it, so normalise or reject it before sorting. Then the product decision rather than the coding one: does a block ending at 10:00 merge with one starting at 10:00? Half-open intervals say yes, because there is no gap a user could book into; a strict greater-than says no. Ask rather than guess.',
