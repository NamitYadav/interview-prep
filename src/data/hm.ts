...
export const hm: Question[] = [
  ...
  },
  {
    ...
    followUps: ['How would you test a host against a remote before deploy?', 'What happens if a remote fails to load at runtime?'],
  },
  {
    id: 'hm-003',
    round: 'hm',
    ...
    followUps: ['How do you handle a team that needs a variant the library does not support?', 'How do you version the library across N consumer apps?'],
  },
  {
    id: 'hm-004',
    round: 'hm',
  ...
  },
  {
    ...
    answer: [
      ...
      'State plainly when Redux (or an equivalent) still earns its place: large teams needing strict, inspectable state transitions with time-travel debugging, or complex cross-cutting client state where implicit prop-drilling or ad hoc context would become unmanageable.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      'Anchor caching to a key strategy, not a timer: cache by [resource + params], with a clear rule for what invalidates it — a mutation touching that resource, a explicit refetch trigger, or time-based staleness for data that tolerates it.',
      ...
      'Name the edge cases that separate a junior and staff-level answer: race conditions between an optimistic update and a slower background refetch, and how you avoid stale data winning the race.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      ...
      'Flag the trap: choosing SSR because it is trendy rather than because the metrics (first paint, TTI, bounce on cold load) actually justify the added operational complexity for this specific user base.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      ...
      'Use memoization deliberately, not everywhere: memoize expensive cell renderers or derived data, but recognize memoization has real limits — it adds its own comparison overhead, so over-memoizing simple components can make things slower, not faster.',
      ...
      'Measure with a real dataset size in mind: performance work validated on 50 rows tells you nothing about behavior at [10,000 rows], so test against representative scale before declaring a fix done.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      'Pick metrics that are both meaningful and stable enough to gate on: bundle size per route and a synthetic Lighthouse/Core Web Vitals score are common choices, since real-user metrics are too noisy to block a merge on directly.',
  ...
  // Testing & quality (6)
  {
    ...
    answer: [
      'Reaffirm the shape and why it still holds at scale: many fast, cheap unit tests for logic and components, a smaller layer of integration tests for how pieces compose, and a thin layer of e2e tests for the critical user journeys only — inverting this ratio is the most common frontend testing failure.',
    ...
    keyPoints: [
      'Reaffirms the pyramid shape and why the ratio matters at scale',
  ...
  },
  {
    ...
    answer: [
      ...
      'Revisit the gate set periodically as the codebase and team grow: what was proportionate at [10 engineers] may be too loose or too strict at [50], so treat the CI pipeline itself as something with an owner and a review cadence.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      ...
      'Pick a concrete mechanism appropriate to the org: a shared schema (OpenAPI/GraphQL schema) validated in both frontends\' and backend\'s CI, or a dedicated consumer-driven contract testing tool, so both sides run against the same source of truth rather than trusting documentation to stay accurate.',
  ...
  },
  {
    ...
    answer: [
      ...
      'Be honest about where the bar is for a given product: a real target ([WCAG 2.1 AA], for instance) stated explicitly, rather than a vague "we care about accessibility" with no measurable standard behind it.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      ...
      'Be equally concrete about the cost: it introduces a new mental model (what runs where), a build/deploy setup that supports it, and constraints on what can live in a server component (no hooks, no browser APIs), which is a real learning curve for a team.',
      ...
      'Flag the immaturity risk honestly: this is a newer pattern with a smaller ecosystem of examples and gotchas than established client-side patterns, so adopting it for a critical B2B product deserves a deliberate pilot, not a wholesale rewrite.',
    ],
    keyPoints: [
      ...
      'Flags ecosystem immaturity as a real adoption risk for critical products',
    ],
  ...
  // Leadership & influence (8)
  {
    ...
    keyPoints: [
      'Clean STAR structure with every specific left as a placeholder',
  ...
  },
  {
    ...
    round: 'hm',
    category: 'Leadership & influence',
    question: 'Tell me about a disagreement with a senior engineer or manager and how it was resolved.',
    answer: [
  ...
  },
  {
    ...
    keyPoints: [
      ...
      'Names a specific mentoring mechanism used, generically',
  ...
  },
  {
    ...
    followUps: ['How did the people affected react?', 'What would you do differently if you saw the same warning signs again today?'],
  },
  ...
  // Delivery & process (6)
  {
    ...
    answer: [
      'Cover the core design: a flag system that supports at least percentage-based rollout, user/segment targeting, and a fast kill switch, with flag state resolved close to the request so a rollback is near-instant rather than requiring a redeploy.',
      'Be explicit about the automatic fallback question specifically: what should happen if the flag service itself is unavailable — a safe, predefined default behavior per flag, not an undefined state that could crash or silently misbehave.',
    ...
    keyPoints: [
      'Covers percentage rollout, targeting, and a fast kill switch as core design',
      'Explicitly designs the automatic-fallback behavior for flag-service failure',
  ...
  // From your CV (8)
  {
    ...
    question: 'Walk me through the React 17 to 18 migration across six apps: how you sequenced it, what broke, what you would do differently.',
    answer: [
      'Structure the walkthrough as a phased plan rather than a single leap: [a dependency and usage audit across the six apps first] to find which ones used APIs affected by the changes (rendering behavior, StrictMode double-invocation, any concurrent-feature-adjacent code), then a deliberately chosen pilot app to build the actual playbook before touching the rest.',
      'Cover sequencing logic explicitly: which app went first and why (lowest risk, still representative), how the remaining five were batched (by risk, by team availability, by shared dependency), and what the rollback trigger was for each batch if something broke post-deploy.',
      ...
      'Close with a genuine "what I\'d do differently": something concrete and non-defensive, like starting the dependency audit earlier, or investing in a shared upgrade checklist before app one instead of discovering the checklist as you went.',
    ],
    keyPoints: [
      ...
      'Describes the shape of real breakage generically, no invented specifics',
      'Ends with a genuine, concrete lesson learned, not a rehearsed non-answer',
    ],
    followUps: ['What would have made you halt the migration partway through?', 'How did you validate each app was stable before moving to the next batch?'],
  },
  {
    id: 'hm-044',
    round: 'hm',
    category: 'From your CV',
    question: 'You cut vulnerability count by roughly two thirds: how did you triage, what did you deliberately leave?',
    answer: [
  ...
  },
  {
    ...
    keyPoints: [
      'Describes flag evaluation architecture generically: store, cache, resolution layer',
  ...
  },
  {
    ...
    round: 'hm',
    category: 'From your CV',
    question: 'You wrote an ADR to consolidate 95+ data grids onto one implementation: how did you phase it and what were the rollback triggers?',
    answer: [
      'Start with why the ADR was needed: [95+ instances built from divergent implementations of essentially the same UI pattern] representing a large, distributed maintenance and inconsistency cost, and the decision being consolidation onto one shared, configurable grid implementation.',
      ...
      'Close with what made this hard in practice: reconciling genuinely different feature needs across [95+] usages without the shared implementation collapsing into an unmanageable pile of configuration flags, and how you drew that line.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      ...
      'Be honest about the real cost: the baseline maintenance overhead (every intentional visual change requires updating snapshots), the false-positive rate from environment or rendering nondeterminism, and the discipline required to keep the team actually reviewing diffs rather than rubber-stamping updates.',
      'Close with whether the cost/benefit held up: state plainly whether you would scope it the same way again, or narrow/widen the coverage given what you learned.',
    ],
    keyPoints: [
      'States a deliberate scope decision, not blanket coverage',
      'Describes the category of bug caught generically, no invented specifics',
      'Honest about real maintenance cost and false-positive risk',
      'Gives an honest retrospective view on whether the scope was right',
    ],
  ...
  },
  {
    ...
    round: 'hm',
    category: 'From your CV',
    question: 'Tell me about the Slack code-review bot: what problem, what metric, did behaviour change?',
    answer: [
      ...
      'Name the metric you\'d track to know if it worked: [review turnaround time, or the age of the oldest open PR], and be candid about the honest form of the answer — behavior change from a nudging tool is usually partial and fades without reinforcement, not a permanent fix on its own.',
    ...
    keyPoints: [
      ...
      'Names a concrete metric it would be judged against',
  ...
  },
  {
    ...
    keyPoints: [
      ...
      'Describes a real tension generically and how it was mitigated',
