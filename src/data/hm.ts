...
export const hm: Question[] = [
  ...
  },
  {
    ...
    answer: [
      ...
      'For server state, use a cache-aware data layer rather than hand-rolling it in a store: TanStack Query where you want something agnostic about the rest of the stack, RTK Query if you are already on Redux Toolkit and want the cache living in the same store, or the router\'s own loaders — React Router, TanStack Router, a framework like Next.js — when the data is route-scoped and you would rather fetch during navigation than after render. All three give you request deduplication, background revalidation and invalidation, which is the part that is genuinely hard to get right by hand.',
      ...
      'Then say when a global store still earns its place, and do not justify it with time-travel debugging — that is a nice property, not a reason. The real case is client state that is cross-cutting and has non-trivial transitions: [a multi-step flow spanning several routes, optimistic or collaborative state with rollback, a permissions layer read everywhere]. Redux Toolkit for that, Zustand where something smaller fits, XState where the transitions are themselves the complexity. If the honest answer is "we needed somewhere to put the API response", the store is the wrong tool and the data layer above is the right one.',
    ],
    keyPoints: [
      'Separates server state from client state as different problems',
      'Names real options for server state — TanStack Query, RTK Query, route loaders — rather than "a library"',
  ...
  },
  {
    ...
    answer: [
      'Choose on the shape of the traffic, not on which technology is more capable: WebSockets do everything SSE does, which is why people default to them and then pay for it. Server-to-client only — status updates, a feed, prices — is SSE: plain HTTP, so it survives proxies and CDNs that mangle upgrades, carries the auth you have, and reconnects on its own. Take WebSockets for low-latency client-to-server messages on the same channel, binary frames, or a constantly changing subscription set. Name SSE\'s limits to stay honest: text only, and the HTTP/1.1 per-origin connection cap that HTTP/2 multiplexing removes.',
      'Treat reconnection as the design, not an error path: the connection will drop — a phone changes network, a proxy times out an idle stream, you deploy. Backoff with jitter is table stakes, and the jitter matters more: without it every client that dropped when you restarted the gateway returns in the same instant and takes it down again. What separates a design from a demo is the resume cursor: the client sends the last event id it processed and the server replays from it — Last-Event-ID on SSE, hand-built on WebSockets.',
      'Put a coalescing layer between socket and React: the failure mode at volume is rendering, not the network — [hundreds of messages a second] with one state update each produces more renders than frames and the tab locks up. The fix is a buffer the socket writes into synchronously, flushed once per frame, keeping only the latest value per key — a symbol updating fifty times between frames costs one update. Connection handling is a day; the render pipeline decides whether the product works.',
      'Finish past the replay window: a sequence number per event so a gap is detected rather than assumed away, a snapshot refetch on a gap rather than a patch, and — since the browser cannot signal backpressure — server-side aggregation on a tick, viewport-scoped subscriptions, and a bounded client buffer.',
    ],
    deeper: [
  ...
  },
  {
    ...
    keyPoints: [
      ...
      'Treats the RUM tool as consent-gated device access, and knows the field data only covers consenting users',
    ],
  ...
  },
  {
    ...
    answer: [
      'Establish what problem forced a document rather than a decision: an ADR settles one choice, a strategy doc sets the frame several years of choices will be made inside. The trigger is usually several teams each making a locally reasonable call that adds up to something incoherent — three data layers, four notification mechanisms — and no single decision fixes it, because the problem is the absence of a shared direction rather than a wrong turn.',
      'Describe the shape of the document concretely. The version that works is short, with four things in it: where the frontend is now stated honestly including what is bad, what the business needs it to support in [two to three years], a small number of bets — three or four, not ten — each with the reasoning that would make it wrong, and a list of non-goals. A strategy nobody re-reads is a document rather than a strategy.',
      'Spend real time on the non-goals, since most candidates skip past them. Naming what you will not do is the part that costs something: it tells a team their pet project is not happening, it closes an option someone senior likes, and it is the only part that frees capacity. Give the form it takes — "we are not building our own component framework, we are not supporting [browser/platform] past [date], we are not moving to [technology] this cycle" — and give the reasoning, because a non-goal without a reason reads as a preference and gets reopened every quarter.',
      'Close on how it was agreed and how you knew it was used. One drafted alone and published is an opinion; one that survives is argued over with the people who live with it, so name who reviewed it, one thing the review changed, and who signed off. Then give evidence it was real — a proposal turned down by pointing at a non-goal — and when you last revisited it.',
    ],
    deeper: [
  ...
  },
  {
    ...
    answer: [
      'Draw the line between mentoring and sponsoring in the first sentence, because they are constantly conflated and the interviewer is listening for the difference: mentoring is advice to a person, sponsoring is spending your own credibility on their behalf in a room they are not in. Sponsorship means you are the one arguing for their promotion, you are the one who looks wrong if the case does not hold, and you accepted that before you started.',
      'Describe how you built the case over time rather than at the deadline, since that separates a real sponsor from someone who wrote a supportive paragraph in the last week. That means arranging the work that would produce the evidence — a project with genuine ambiguity and visible ownership, a piece of scope that crosses a team boundary, a chance to present their own work to people who will later be in the room — and writing things down as they happened rather than reconstructing them later.',
      'Be specific about what counts as evidence at the level in question, because calibration rooms discount adjectives and accept artifacts. Strong evidence is the design doc they wrote and the objections they handled, the incident they ran, the migration they sequenced, peer feedback naming a specific instance rather than praising generally, and — critically — evidence of scope beyond their own team, since that is usually the actual bar being argued about. Weak evidence is volume of tickets, tenure, and how hard they work.',
...
a:hover { color: blue; }`,
    answer: [
      'Diagnose before prescribing, and be precise about which rule loses to what — that is the trap here. The first rule is one ID, two classes and three element types, so it outranks the plain .nav-link rule on specificity. It does not outrank the is-active rule: the cascade settles origin and importance before it ever reaches specificity, so an !important declaration wins no matter how specific anything else is. The author reached for it, it worked, and the real problem is still sitting there.',
      'Give the order out loud, since that is what makes this a diagnosis rather than a guess: origin then importance, then cascade layers, then specificity, then source order — and specificity is three counters, IDs then classes and pseudo-classes then element types, compared left to right, which is why no quantity of classes outweighs one ID.',
      'Then the most diagnostic finding, the one people miss: a:hover is one pseudo-class and one element type, so inside the sidebar it loses to that ID chain on every nav link. It is a dead rule — the hover colour never applies, and nobody will work out why by reading the hover rule.',
      'The actual fix: flatten the first selector to the class it is really targeting. Specificity is not a score to win but a signal of how tightly a rule is bound to a place in the DOM, and a five-part selector claims a link only looks like this inside that exact structure. Once it is flat, source order decides, the hover rule starts working, and !important is unnecessary.',
    ...
    keyPoints: [
      ...
      'Gives the cascade order out loud and describes specificity as three counters compared left to right, not one number',
...
.modal {
  ...
  z-index: 9999;
    ...
    answer: [
      'Open with the outright bug rather than the smells. .header is position: static, so it is still in the flow and still occupies its 64px. .content then adds another 64px of margin-top below it and claims 100vh minus 64px, which puts the bottom of the content 64px past the viewport and gives the page a scrollbar nobody intended. The arithmetic only works if the header is fixed or absolute, so ask which is missing, the position or the margin.',
      'The modal has the matching defect: top and left are both 50%, but only left is corrected, by the -200px margin pulling it back half its width. There is no vertical offset, so it is horizontally centred and vertically not — its top edge sits on the midline and it hangs below, off-screen on a short viewport. It looks fine on the one screen it was built on, which is why it survives review.',
      'Then the coupling: 64px appears three times and every occurrence has to change together. A custom property makes the relationship explicit and the next header redesign a one-line change instead of a grep — and one of those three is already wrong.',
      'Raise 100vh as a mobile correctness issue: vh does not account for the collapsing browser chrome, so the content area is taller than the visible viewport and the bottom is cut off exactly where the important controls sit. The dynamic viewport units exist for this.',
      'The z-index is the smallest but most diagnostic point: 9999 means there is no layering scale, only escalation. What makes it suspect rather than ugly is that z-index only orders an element among its siblings inside its own stacking context — so if any ancestor forms one, the whole subtree paints as a unit and nothing here lifts it above anything outside. What creates one: a positioned element with a z-index other than auto, opacity below 1, transform, filter, mix-blend-mode, isolation, containment.',
    ],
    deeper: [
...
@media (max-width: 768px) {
  .card { width: 100%; }
    ...
    answer: [
      'Diagnose the missing third card first because it is the reported bug, and give the mechanism rather than the symptom: with default content-box sizing, the 32px of horizontal padding is added outside the declared 33% width, so each card occupies 33% plus 32px of the row. Three of those exceed the container, and because the container wraps, the third card does not overflow — it wraps onto the next line, which is why you see two per row. box-sizing: border-box brings each card back to exactly 33% including its padding and the bug is gone. Whether that is a global rule or a per-component one depends on what the codebase already does.',
      'Then explain the cramped range, and correct the framing while you do: the media query stops applying at 768px, so from 769px upward each card is a third of the width — roughly 256px minus padding at the low end, which is where it feels cramped. There is no upper bound to the problem; it just gradually stops hurting as the viewport grows, and there is no width at which the three-column choice was justified by the content. That is the structural weakness of hardcoded breakpoints: the layout is pinned to viewport widths rather than to what the card needs.',
      ...
      'Leave two smaller comments rather than expanding the review: 12px is below a comfortable reading size and, being in px, ignores a user who has raised their browser font size, so rem is the more respectful unit here. And max-width breakpoints run against a mobile-first codebase if the rest of the project uses min-width.',
    ],
    deeper: [
      'On the breakpoint itself: the answer is not to add a second media query at whatever width it stops looking bad on your monitor, which is how a stylesheet accumulates breakpoints nobody can justify afterwards. Tie the change of layout to what the content needs — a container query, or the intrinsic grid above — and the question of which viewport widths to test stops mattering.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    answer: [
      'Get the legal shape right first — the architecture follows from it. Two regimes apply, and they are not the same. The ePrivacy rule — in Germany §25 TDDDG — governs storing or reading anything on the user\'s device: cookies, localStorage, IndexedDB, a cache entry, a fingerprint. It requires prior consent whether or not the data is personal, with a narrow strictly-necessary exception. GDPR then governs what you do with that personal data. So "we use legitimate interest for analytics" does not get you past the access-to-device question.',
      'Make the gate architectural rather than a matter of discipline. The failure mode is always the same: a tag fires on page load because someone added it to the head and no review catches it. Nothing touching storage or a third party may be imported for its side effects at module scope — every gated integration loads dynamically, after a consent signal resolves, from a single source-of-truth consent state. The strongest version resolves consent before the bundle that could violate it runs, with CSP connect-src as a backstop. A banner over a page that has already loaded analytics is theatre.',
      'The genuinely staff-level part is testing the denied path. Nobody uses it in development, so it is the one that breaks, and invisibly — the product still works, it just silently violates. Concretely: an automated check that loads the app, declines everything, and asserts no request went to a non-essential origin and storage holds nothing beyond the allowlist; the same for withdraw-after-accept; and the denied state exercised in normal QA, not only in compliance review.',
      'Two things before they are asked. Consent Mode is not an exemption: basic blocks the tag from loading, advanced loads it cookieless, and v2 adds ad_user_data and ad_personalization to the original two signals, all defaulted to denied before init. And region resolves at the edge, not in the client, with withdrawal as reachable as consent and actually clearing what was stored.',
    ],
    deeper: [
  ...
  },
  {
    ...
    answer: [
      'Correct the usual misconception before explaining the mechanism, because it is what the question is really testing: CORS is not a protection your server gets, it is a relaxation of a restriction the browser imposes. The same-origin policy stops one origin reading another\'s responses; CORS is how a server opts into being read. It does not stop the request arriving and protects the server from nothing — a non-browser client ignores it entirely. So "we fixed it with CORS" for a security problem has usually fixed nothing.',
      'Describe the preflight precisely. For a request that is not simple — anything beyond GET/HEAD/POST, or carrying a header outside the small safelisted set, or a content type other than form-encoded, multipart, or text/plain — the browser sends an OPTIONS first carrying Access-Control-Request-Method and Access-Control-Request-Headers, and it will not send the real request unless the response allows that exact method and those exact headers. That is why adding one custom header to an existing call produces a failure that looks like the server is down: the real request never left. Credentials are their own trap — in credentials mode include, the server must answer a specific origin rather than a wildcard, and set Vary: Origin so a cache cannot cross-serve.',
      'Then draw the line to clickjacking, because it is a different problem filed in the same drawer. It does not involve reading your responses at all — the attacker frames your real page, renders it invisible over their own UI, and the user clicks your button believing it was something else. CORS is irrelevant: the page loads normally and the click is genuine. The control is telling the browser who may frame you, which is CSP\'s frame-ancestors — an origin list, superseding X-Frame-Options and its never-reliable ALLOW-FROM, with the old header kept only as a fallback for genuinely old clients.',
    ],
    deeper: [
      ...
      'For a product where a wrong click costs money — usually the case the interviewer has in mind — that means framing protection on every authenticated origin by default rather than per page, and for the genuinely irreversible actions, a transfer, a payee change, a limit increase, a confirmation that cannot be satisfied by a single click landing in the right place, which in practice means re-authentication or a step the framing attacker cannot fake. Defence in depth rather than one header, and worth saying which of the two you would treat as the actual control.',
    ],
    keyPoints: [
      ...
      'Gets credentials mode right: a specific origin rather than a wildcard, with Vary: Origin set',
  ...
  // Regulated & payments FE (7)
  {
    ...
    answer: [
      'Frame what SCA demands before touching the UI: PSD2 requires two independent factors from knowledge, possession and inherence for in-scope EEA payments, and 3-D Secure 2 carries it for cards. Everything difficult follows from one architectural fact — the authentication happens on the issuer\'s Access Control Server, a page you do not control, cannot style, and cannot rely on. And it is a branch rather than a step: 3DS2 sends risk data up front and most transactions come back frictionless.',
      'Choose the iframe challenge or a full redirect on the honest trade-off. The iframe keeps your page alive so no state is lost, but you are embedding a document you cannot inspect, from issuers whose pages often are not responsive or bust out of the frame. The redirect is uglier and destroys in-memory state but works everywhere, so offer the iframe and fall back on a timeout or frame error. A popup is not the answer: by the time the PSP says a challenge is coming, the user gesture that would have beaten the blocker is gone.',
      'Make state recovery a server-side concern, which is what the question is really probing. Do not serialise the cart into the return URL or trust sessionStorage across the round trip — the ACS return is typically a form POST, it may land in a different tab, and a cross-site POST can lose the SameSite=Lax cookie. Create a payment intent server-side before the challenge, hold every piece of state that matters against it, and put nothing in the return URL but the intent id.',
    ...
    keyPoints: [
      ...
      'Makes the return page a pure function of server-owned status, reading state rather than re-submitting',
  ...
  },
  {
    ...
    answer: [
      'State the goal correctly: not to secure card data well, but to never possess it. Scope follows the data: if the card is entered into a document served by your PSP, the systems that must meet the full standard are theirs. The mechanism is hosted fields: the PSP serves each card input as a cross-origin iframe and returns a token your server can charge, and same-origin policy enforces the isolation rather than your team remembering not to look.',
      'Be precise about the two SAQ levels — that is what the architecture buys. SAQ A is for a merchant who has fully outsourced the payment page — a redirect, or fields delivered entirely by the PSP in iframes. SAQ A-EP is for a site that does not receive card data but controls the page affecting the security of the transaction — what you get if you serve the form, or the script that collects the fields. A-EP carries several times the requirements of A, so "we use a PSP" settles nothing.',
      'Treat CSP as a scope and integrity control, and tie it to what PCI DSS v4 now requires of payment pages: an inventory of every script with a justification, assurance of each script\'s integrity, and tamper detection on the page\'s content and headers — 6.4.3 and 11.6.1, mandatory since March 2025. They bind differently — the other half of the SAQ distinction: v4.0.1 swapped them for an eligibility attestation under SAQ A in January 2025; A-EP carries them in full. Concretely: strict script-src on the checkout route, no unsafe-inline, SRI on CDN scripts, locked-down connect-src and form-action, and reports a human reads.',
  ...
  },
  {
    ...
    answer: [
      'Name the actual problem first: a timeout tells you nothing about whether the server processed the request. The transfer may have completed and the response been lost, or never arrived. The frontend cannot distinguish these, so any design deciding from the client\'s view of the network is wrong. What makes it safe is that the server can recognise a repeat of the same intent — that is what an idempotency key is for.',
      'Be precise about where the key comes from and what it identifies. It is generated on the client once, when the user forms the intent — when the confirm screen opens, not when the request is dispatched — and stays fixed across retries of that intent. A key per request turns retries into distinct payments — the bug the mechanism exists to prevent. It must change when the intent changes: editing the amount is a new intent and a new key, or the server replays the first result and the user never sends what they meant.',
      'Then answer what the question is really about: what the UI shows while the outcome is unknown. Not a spinner, and not a retry button — both invite a second submit. The submit is disabled after the first press and stays disabled, the screen moves to an explicit pending state saying the payment is not yet confirmed, and the app resolves it by asking the server — polling the intent, or a status channel — rather than inferring from the failed request. Unresolved, it sits in their activity as pending, not vanished.',
    ...
    keyPoints: [
      ...
      'States the server contract explicitly rather than assuming it: a repeated key replays the original response, the same key with a different payload is rejected',
      ...
      'Retries only network errors and 5xx, with backoff and a cap, rather than retrying forever',
    ],
  ...
  },
  {
    ...
    answer: [
      'Start with the rule and the reason: money is an integer count of minor units carried with its currency code, never a float. IEEE 754 cannot represent most decimal fractions exactly, which is why 0.1 plus 0.2 is not 0.3 in any language using it — a defect class, not a curiosity. Sum a few hundred line items in floats and the total is a fraction of a unit off, the UI rounds it away, and it surfaces weeks later in a reconciliation nobody can explain.',
      'Get the minor-unit detail right, because "multiply by 100" is the version that ships bugs: the number of decimal places is a property of the currency, not a constant. Most are two, JPY has none, and KWD, BHD and TND have three. So a money value is a pair of amount and currency, and the exponent comes from the currency. Over the wire, an integer minor-unit amount or a decimal string, never a JSON number with a fraction, because parsing that back is a float again.',
      'Separate display from storage cleanly and put Intl.NumberFormat on the display side only. It handles the things people hand-roll wrongly — symbol versus code, which side the symbol sits on, the grouping and decimal separators, how many decimals this currency actually uses, and negative-value conventions that vary by locale. Construct the formatter once rather than per row, because it is expensive and this runs in tables, and note that locale and currency are independent: a German user looking at a dollar balance wants German grouping with a dollar amount.',
      'Two rules follow. Rounding direction and rounding point are decisions someone has to state — half-up, half-even, or toward the customer, and at which step — and in a regulated product they belong in the server\'s domain rules with the client showing the outcome. And never parse a formatted string back into a number, because the same digits mean different values in different locales.',
    ],
    deeper: [
    ...
    keyPoints: [
      ...
      'Refuses to parse a formatted string back into a number, since the same digits mean different values per locale',
    ],
  ...
  },
  {
    ...
    answer: [
      'Establish what the control is for before designing screens, because it determines what the UI owes the approver: dual control exists so that no single person can both create and authorise a consequential change. The second person is not rubber-stamping a request, they are independently confirming a decision — and a UI that makes approving easier than understanding has defeated the control while appearing to satisfy it.',
      'Model it as an explicit state machine rather than a boolean — the intermediate states are where the real requirements live: drafted, submitted, approved, executed, rejected, expired. The payload is frozen at submission — the maker cannot edit it while it sits pending, because otherwise the thing approved is not the thing that executes; a change sends it back to draft and invalidates the approval. And approval is not execution: approving records a decision, execution is a separate step with its own outcome, so an approved item that fails to execute is visible as exactly that.',
      'Be concrete about what the approver is shown, because this is the question. Not the maker\'s form — a review view built for checking: the full payload as it will execute, a diff against current state for anything changed rather than created, aggregates that make the scale legible (total amount, recipient count, how many are new payees), and anything already known to be unusual flagged inline. Who submitted it and when. For a bulk item, the actual rows rather than a summary, because approving a count is not approving the content — and the amounts shown are the exact ones that will move, not rounded display values.',
    ...
    keyPoints: [
      ...
      'Enforces no-self-approval and approval limits on the server, not by hiding a button',
  ...
  },
  {
    ...
    answer: [
      'Start from why a live session is not sufficient, because it explains the whole design: a session proves the user authenticated at some point, not that the person at the keyboard right now is them. The risk is precisely the one a stolen or unattended session creates — an attacker adds a payee they control and drains the account. Step-up authentication re-establishes presence at the moment of the consequential action, and under PSD2 adding a beneficiary attracts SCA in its own right, not merely as internal policy.',
      'Design around session freshness as a first-class concept rather than a boolean logged-in flag. The session carries the time authentication actually happened, and each sensitive action declares how fresh it needs that to be and at what strength — a limit change might accept the last few minutes, while adding a payee always challenges. In OIDC terms that is auth_time with max_age and acr_values, and the point is that the requirement is per action, so a new sensitive action is a declaration rather than a bespoke flow.',
      'Make the elevation specific to the operation, which is what separates a real design from a flag. What comes back from a successful step-up is a short-lived token bound to that particular action — the payee being added, the limit being set — and the server checks it against the operation it is executing. A general "elevated for ten minutes" flag is much weaker, because anything triggered in that window inherits it. Bound, single-use, short-lived, validated server-side; the frontend requesting a challenge is UX, never enforcement.',
    ...
    keyPoints: [
      ...
      'Preserves unsubmitted work when a challenge fails or is abandoned',
    ],
  ...
  },
  {
    ...
    answer: [
      'Answer the second half first, because most candidates skip it: the frontend is never the system of record. It runs on a device you do not control, in a browser the user can modify, over a connection that drops, and anything it reports can be forged or lost. So every event with legal or regulatory weight is recorded by the service that performed the action, at the moment it performed it. What it is genuinely for is the context the backend cannot observe.',
      'Name that context: the backend knows a transfer was made, but only the client knows what the user was shown before confirming — the exact amount and fee displayed, the rate quoted and its expiry, which risk warning or disclosure was on screen, which consent variant was presented, which version of the terms rendered. In a dispute the question is usually not whether the action happened but what the customer was told before it did — a frontend fact.',
      'Apply data minimisation as a design rule, not a review step, because this is where a frontend audit trail becomes a liability. Log the identifier and the event, not the value: a payee id rather than an IBAN, that a field was corrected rather than its contents. Card data, credentials, OTPs and full account numbers never appear, and the enforcement is a typed event schema with an allowlist of fields rather than free-form objects — plus real care with session replay and error reporting, which capture everything by default and are almost never in anyone\'s data map.',
  ...
  },
  {
    ...
    answer: [
      ...
      'Only after mitigating impact (rollback, feature-flag kill switch, or a scoped hotfix) do you move to root-causing properly, and you explicitly separate "make it stop" from "understand why," since conflating them under pressure is how bad, rushed fixes ship — and you hold that separation more strictly, not less, when you were paged at 3am and your judgement is degraded.',
  ...
  // Internationalisation (2)
  {
    ...
    answer: [
      'Lead with the thing that breaks first, which is string concatenation, because it is the habit that makes everything else impossible. A sentence assembled from fragments encodes English word order and English grammar, and no translation of a fragment works once the target language reorders the clause or inflects the noun. The unit of translation is a whole message with named placeholders, and the moment a message contains a number, a gender or a condition, that logic belongs inside the message rather than in the component. That is what ICU MessageFormat is for.',
      'Get plurals right specifically, because English trains people to believe there are two cases. CLDR defines up to six plural categories and languages use different subsets: English has one and other, Polish has one, few, many and other, Arabic uses all six, Japanese has only other. So a ternary on count equals one is a bug in most of the EU, and Intl.PluralRules or an ICU plural block is the correct mechanism. The related trap is a select for grammatical gender, which has no English equivalent and is invisible until a translator raises it.',
      'Put every formatted value through the Intl APIs rather than anything hand-written, because this is the part that silently looks wrong rather than breaking: NumberFormat for grouping and decimal separators — a German user reads one thousand as 1.000,00, so a hand-rolled formatter shows them a different number, not a differently styled one — DateTimeFormat for date order and for an explicit time zone rather than the device default, RelativeTimeFormat, ListFormat for the right conjunction, and Collator for sorting, since a default string comparison puts every accented character in the wrong place.',
      'Then two things that are cheap now and expensive later: design for text expansion, since German and Finnish run substantially longer and buttons, tabs and table headers break first, and use CSS logical properties throughout so direction is a document attribute rather than a stylesheet fork.',
    ],
    deeper: [
    ...
    keyPoints: [
      ...
      'Designs for text expansion, naming German and Finnish and the components where it actually breaks',
      'Uses CSS logical properties so direction is a document attribute rather than a stylesheet fork',
    ],
  ...
  },
  {
    ...
    answer: [
      'Make extraction automatic and non-negotiable, because a pipeline that depends on someone remembering to add a string will always be incomplete. Strings are authored in code inside the translation function, a build step extracts them into the source catalogue, and CI fails if the catalogue is stale or a literal appears where a translated string should be. The lint rule that catches a hardcoded string in JSX is worth more than any process, because the failure it prevents is the one that actually happens.',
      'Take a position on keys versus source strings — the interviewer is checking whether you have felt the trade-off. English text as the key makes the code readable, but a typo fix silently orphans every translation of that string. Structured keys survive edits and distinguish two identical English words that translate differently — a noun and a verb both spelled "order" in English and never in German — but they drift and need discipline. The workable answer is structured keys plus the source string and a description as context, because a translator working from a bare key produces a confident wrong translation and nobody finds out.',
      'Put pseudo-localisation in CI, the cheapest real check available: render every string accented, padded to the length of the longest expected language, and bracketed. Anything still in plain ASCII was never extracted, anything overflowing will overflow in German, anything with a missing bracket was concatenated. Then ownership, which is what the question is really asking: engineering owns the pipeline and the source strings, product or content owns the wording, professional translators own the target languages — and someone merely fluent reviewing on top of their day job is where quality silently drops.',
      'Two consequences: each key ships with a description, placeholder types and a character budget, not the key alone, and message bundles split by route with an explicit fallback chain resolved before first paint, so there is no flash of the fallback language.',
    ],
    deeper: [
    ...
    keyPoints: [
      ...
      'Names the translator context that is actually needed: a description, placeholder types and a character budget',
