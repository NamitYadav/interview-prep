...
export const hm: Question[] = [
  ...
  },
  {
    ...
    answer: [
      ...
      'Describe the RCA method concretely: trace back through the actual sequence of events in a five-whys-style pass rather than stopping at the first proximate cause, and separate what caused the issue from what allowed it to reach production undetected — usually two different findings, both worth fixing.',
...
.modal {
  ...
  z-index: 9999;
    ...
    answer: [
      'Open with the outright bug rather than the smells. .header is position: static, so it is still in the flow and still occupies its 64px. .content then adds another 64px of margin-top below it and claims 100vh minus 64px of height, which puts the bottom of the content 64px past the viewport and gives the page a scrollbar nobody intended. The arithmetic only works if the header is fixed or absolute, and nothing here makes it so — so ask which is missing, the position or the margin, because the author is the only one who knows.',
      'The modal has the matching defect: top and left are both 50%, but only left is corrected, by the -200px margin pulling it back half its width. There is no vertical offset at all, so it is horizontally centred and vertically not — its top edge sits on the midline and it hangs below, and on a short viewport the bottom is off-screen. It looks fine on the one screen it was built on, which is exactly why it survives review.',
      'Then the coupling: 64px appears three times and every occurrence has to change together. A custom property makes the relationship explicit and the next header redesign a one-line change instead of a grep — worth pointing at the three places that silently drift apart, one of which is already wrong.',
      ...
      'Treat the z-index as the smallest but most diagnostic point: 9999 means there is no layering scale, only an escalation, and the next overlay will be 10000. What makes the number suspect rather than merely ugly is that z-index only orders an element among its siblings inside its own stacking context, so if any ancestor of this modal forms one, the whole subtree paints as a unit and no value here lifts it above anything outside. What creates one: a positioned element with a z-index other than auto, opacity below 1, transform, filter, mix-blend-mode, isolation, containment. You find the culprit by walking up the ancestors in devtools.',
      'Only if asked: on the centring technique, the negative margin only works because the width is hardcoded, and the hardcoded width is itself the problem on a narrow screen. A transform-based or grid-based centre fixes the vertical axis and the coupling in one move and works at any width, and a max-width with a percentage keeps it on screen at 320px.',
      'Only if asked: position: fixed has the same family of surprise. It is normally relative to the viewport, but an ancestor with a transform, a filter, will-change on either, or containment establishes a containing block and silently captures it — so a dialog can land somewhere unrelated to the viewport because someone added a transform for an animation three levels up.',
      'Only if asked: the structural fix is not a bigger number — render the modal in a portal at the top level of the document so it is trapped in neither a stacking context nor someone else\'s containing block, and keep a small named scale for the few layers that genuinely exist. Ask whether the project already has one, since introducing a scale is a codebase-wide conversation rather than a comment on this file.',
    ],
    keyPoints: [
...
@media (max-width: 768px) {
  ...
  },
  {
    ...
    answer: [
      'Get the legal shape right first, because the whole architecture follows from it and most implementations are built on the wrong model. Two regimes apply and they are not the same one. The ePrivacy rule — transposed in Germany as §25 TDDDG — governs storing or reading anything on the user\'s device: cookies, localStorage, sessionStorage, IndexedDB, a cache entry, a device fingerprint. It requires prior consent whether or not the data is personal, with a narrow strictly-necessary exception. GDPR then governs what you do with personal data once you have it, and gives you other legal bases. So "we use legitimate interest for analytics" does not get you past the access-to-device question, and an architecture built on that assumption is wrong before it ships a line of code.',
      'Make the gate architectural rather than a matter of discipline, since the failure mode is always the same: a tag fires on page load because someone added it to the head, and no code review catches it. Nothing that touches storage or a third party may be imported for its side effects at module scope — every gated integration is loaded dynamically and only after a consent signal resolves, and the consent state is the single source of truth that those loaders read. The strongest version resolves consent before the bundle that could violate it executes at all, with CSP connect-src as a backstop so anything that slipped through fails loudly rather than silently phoning home. A consent banner rendered over a page that has already loaded analytics is theatre.',
      ...
      'Only if asked: handle Google Consent Mode accurately, because it is the piece most often described wrongly in an interview. It does not exempt anything, and two separate things get conflated. Basic versus advanced is what decides whether the tag loads at all before a decision: basic blocks the load until consent arrives, advanced loads the tag but runs it in a restricted, cookieless state that sends pings carrying no identifiers. v2 is a different axis entirely — it added two signals, ad_user_data and ad_personalization, on top of the original ad_storage and analytics_storage, so there are four states to set, not two. Either way you must set the defaults to denied before the tag initialises, or the first request goes out as granted. Explain what the mode actually buys — modelled conversions and a documented signal path to Google — and what it does not, which is the §25 question, since the tag still runs. It is a way of degrading gracefully, not a legal basis.',
      'Only if asked: design for regional variation without letting it become a maze. The realistic shape is one consent engine with a region-resolved policy: EU/EEA and the UK require opt-in before any non-essential storage, other jurisdictions may permit opt-out or notice-only, and the resolution happens server-side or at the edge where you can see the request, because geolocating in the client means the page has already loaded. Two details separate a real design from a diagram: the choice must be as easy to withdraw as to give, which means a persistent, reachable control rather than a one-time banner, and withdrawal has to actually do something — clear what was stored under the old consent and stop the integrations, not merely flip a flag for next time. Keep a record of what was consented to, when, and against which policy version, because a consent you cannot evidence is one you do not have.',
    ],
    keyPoints: [
      ...
      'States Consent Mode accurately: basic vs advanced decides whether the tag loads at all, v2 is the two added signals (ad_user_data, ad_personalization) alongside ad_storage and analytics_storage, defaults denied before init, and none of it is an exemption',
  ...
  // Regulated & payments FE (7)
  {
    ...
    answer: [
      'Frame what SCA actually demands before touching the UI, because it constrains the design: PSD2 requires two independent factors from knowledge, possession and inherence for in-scope electronic payments in the EEA, and 3-D Secure 2 is the protocol that carries it for card payments. The key architectural fact is that the authentication does not happen in your application — it happens on the issuer\'s Access Control Server, a page you do not control, cannot style, and cannot rely on to behave well. Everything difficult about this flow follows from that. And the challenge is not certain: 3DS2 sends risk data up front, most transactions come back frictionless, and exemptions apply, so the UI treats the challenge as a branch that sometimes fires rather than a step in a linear flow.',
      'Choose between the iframe challenge and a full redirect on the honest trade-off rather than on which looks nicer. The iframe keeps the user inside your app and your page alive, so no state is lost — but you are embedding a third-party document you cannot inspect, at a fixed challenge window size, and issuer pages frequently are not responsive, break inside a frame, or try to bust out of it. The redirect is uglier and destroys in-memory state, but it works everywhere. So offer the iframe and fall back to redirect on a timeout or frame error; on mobile web the answer often just is redirect. Say out loud why a popup is not the answer: one opened inside the click handler would survive the blocker, but you do not know a challenge is coming until the PSP has answered, and by then the user gesture is gone — so the window you would open at that point is exactly the one the browser blocks.',
      'Make state recovery a server-side concern, which is the answer the question is really probing. Do not serialise the cart into the return URL or lean on sessionStorage surviving the round trip — the return from the ACS is typically a form POST, it may land in a different tab, and a cross-site POST can lose the SameSite=Lax cookie you were counting on. The durable shape: create a payment intent server-side before starting the challenge, hold every piece of state that matters against it, and put nothing in the return URL but the intent id. On return the client does one thing, fetch the authoritative status of that intent, and renders from that. A client-reported success is not evidence of anything.',
      'Only if asked: make the resume idempotent, because this path is re-entered more often than anyone expects: the user hits back, refreshes the return page, has two tabs open, or the challenge completes while the tab is backgrounded and the app reconnects. Resuming means reading state, never re-submitting. The intent carries a status the server owns (requires_action, processing, succeeded, failed), and the return page is a pure function of that status — so a second load of the same URL shows the same result rather than starting a second payment. Pair that with a submit that carries an idempotency key, so even a genuine retry cannot double-charge.',
      'Only if asked: the states the happy path forgets are where a payment product earns or loses trust. There is a real window where the payment is neither confirmed nor failed and you must show something honest rather than a spinner forever — a pending state, a poll or a push channel, and a rule for what happens when the challenge is abandoned or times out on the issuer side. Do not present an unknown outcome as either success or failure, do not offer a retry button that could produce a second charge, and make sure the pending state is reachable again if the user simply closes the tab and comes back.',
    ],
    keyPoints: [
  ...
  },
  {
    ...
    keyPoints: [
      ...
      'Uses CSP script-src, SRI, connect-src and form-action as scope controls, and gets 6.4.3/11.6.1 right: SAQ A v4.0.1 replaced them with an eligibility attestation, while SAQ A-EP carries them in full',
